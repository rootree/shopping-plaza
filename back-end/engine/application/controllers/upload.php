 ...
 * @author     Ivan Chura
 */
class Upload_Controller extends Web_Controller {
    ...
    public function index() {
        ...
        define('NICUPLOAD_PATH', IMAGES_COMMON_ON_DISC); // Set the path (relative or absolute) to
        // the directory to save image files

        define('NICUPLOAD_URI', IMAGES_COMMON_IN_WWW);   // Set the URL (relative or absolute) to
        ...
        if($_SERVER['REQUEST_METHOD']=='POST') { // Upload is complete
            ...
            $id_for_item = intval($this->uri->segment('itemid'));
            $type = ($this->uri->segment('type'));

            if( empty($type) ) {
                nicupload_error('Invalid parameters'  );
            }
            ...
            $content = array(
                "for_item" => $id_for_item,
                "type" => $this->convertSelectedPageToCode($type),
                "img_size" => filesize($image),
                'firm_id' => $this->firmID
            );

            $statusIN = $this->db->insert('images', $content);
            if(!count($statusIN)){

                nicupload_error('DB error');

            }

            $imageId = $statusIN->insert_id();

            $targetFile = SuperPath::get($imageId, false, IMAGES_TYPE_COMMON)  ;
...
/*
            $cmd_command = 'composite -gravity SouthWest -dissolve 35 ' . escapeshellarg(WATER_MARK) .
...
*/
            ...
            $status['done'] = 1;
            $status['width'] = $size[0];
            $status['url'] = SuperPath::get($imageId, true, IMAGES_TYPE_COMMON).'.jpg';
...
function nicupload_error($msg) {
    echo nicupload_output(array('error' => $msg)); exit();
}
