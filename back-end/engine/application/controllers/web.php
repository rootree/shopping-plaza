...
abstract class Web_Controller extends Template_Controller {
    ...
    public function __construct() {
        ...
        if($this->session->get("access") && $this->session->get("access") != ACCESS_GUEST){
            ...
            if(!isset($this->firm->YMLenabled)){
                url::redirect(url::site() . "login");exit();
            }
			...
			if(!empty($lastPage) && !$lastRedirected && !array_key_exists('redirected', $_REQUEST)){
				$this->session->set("lastRedirected", 1);
				url::redirect(url::site() . substr($lastPage, 1)."?redirected");exit();
			}
    ...
    protected function convertSelectedPageToCode($page) {
        switch($page){
            case PAGE_LOGIN: return 1;
            case PAGE_DASHBOARD: return 2;
            case PAGE_REG: return 3;
            case PAGE_SETTINGS: return 4;

            case PAGE_ITEMS: return 5;
            case PAGE_USERS: return 6;
            case PAGE_CLIENTS: return 7;
            case PAGE_ORDERS: return 8;
            case PAGE_FEEDBACK: return 9;
            case PAGE_VACANCY: return 10;
            case PAGE_NEWS: return 11;
            case PAGE_PARTNERS: return 12;
            case PAGE_PAGES: return 13;
            default: return 0;
        }
    }
