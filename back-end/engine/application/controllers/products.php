...
class Products_Controller extends Web_Controller
{
    ...
    public function index()
    {
        ...
        if (!$this->cats->count()) {
            ...
            url::redirect(url::site() . "settings/catsadd?emptyCat");
            return;
    ...
    public function excel()
    {
        ...
        $row++;
        if (!empty($description)) $worksheet->write($row, $column, ($description), $sheetTitleAddon);
        $row++;
        $worksheet->write($row, $column, ($prise), $sheetTitleAddon);
        $row++;

        $worksheet->writeUrl($row, $column, 'http://' . $this->firm->domain . '/', ($this->firm->domain), $sheetLink);
        $row++;
        if (!empty($mail)) $worksheet->write($row, $column, ($mail), $sheetTitleSubAddon);
        $row++;
        if (!empty($address)) $worksheet->write($row, $column, ($address), $sheetTitleSubAddon);
        $row++;
        if (!empty($WorkFlow)) $worksheet->write($row, $column, ($WorkFlow), $sheetTitleSubAddon);
        $row++;
        if (!empty($Phones)) $worksheet->write($row, $column, ($Phones), $sheetTitleSubAddon);
        $row++;
        if (!empty($fax)) $worksheet->write($row, $column, ($fax), $sheetTitleSubAddon);
        $row++;
        ...
        foreach ($this->groups as $group) {
            ...
            $worksheet->write($row, $columnDF, iconv('UTF-8', "CP1251", ''), $columnTitleFormatBor);
            $columnDF++;
            ...
            foreach ($this->items as $item) {
                ...
                if ($item->cat_id == $group->cat_id) {
                    ...
                    $worksheet->write($row, $column, iconv('UTF-8', "CP1251", strip_tags($item->subTitle)), $regularFormat);
                    $column++;
                    $model = iconv('UTF-8', "CP1251", strip_tags($item->title));

                    $worksheet->writeUrl($row, $column, 'http://' . $this->firm->domain . '/products/item/id/' . $item->product_id, $model, $sheetLink);
                    $column++;
                    // $worksheet->write($row, $column, iconv('UTF-8', "CP1251", strip_tags($item->desc_mini)), $regularFormat); $column ++;
                    $worksheet->write($row, $column, iconv('UTF-8', "CP1251", strip_tags($item->desc_mini)), $regularFormat);
                    $column++;

                    $worksheet->write($row, $column, iconv('UTF-8', "CP1251", money::ru($item->price)), $regularFormatCenter);
                    $column++;

                    if ($this->firm->prices->enabled & 1 && $this->firm->prices->visible & 1) {
                        $worksheet->write($row, $column, iconv('UTF-8', "CP1251", money::ru($item->price1)), $regularFormatCenter);
                        $column++;
                    }
                    if ($this->firm->prices->enabled & 3 && $this->firm->prices->visible & 3) {
                        $worksheet->write($row, $column, iconv('UTF-8', "CP1251", money::ru($item->price2)), $regularFormatCenter);
                        $column++;
                    }
                    if ($this->firm->prices->enabled & 4 && $this->firm->prices->visible & 4) {
                        $worksheet->write($row, $column, iconv('UTF-8', "CP1251", money::ru($item->price3)), $regularFormatCenter);
                        $column++;
                    }
                    if ($this->firm->prices->enabled & 8 && $this->firm->prices->visible & 8) {
                        $worksheet->write($row, $column, iconv('UTF-8', "CP1251", money::ru($item->price4)), $regularFormatCenter);
                        $column++;
                    }
                    if ($this->firm->prices->enabled & 16 && $this->firm->prices->visible & 16) {
                        $worksheet->write($row, $column, iconv('UTF-8', "CP1251", money::ru($item->price5)), $regularFormatCenter);
                        $column++;
                    }
