...
var nicEditorConfig = bkClass.extend({
	...
	buttonList : [ 'bold','italic','underline','hr', 'left','center','right','justify','ol','ul','fontSize','fontFamily' ,'indent','outdent','image','upload','link','unlink','forecolor','bgcolor', 'xhtml'],
	iconList : {"xhtml":27,"bgcolor":1,"forecolor":2,"bold":3,"center":4,"hr":5,"indent":6,"italic":7,"justify":8,"left":9,"ol":10,"outdent":11,"removeformat":12,"right":13,"save":24,"strikethrough":15,"subscript":16,"superscript":17,"ul":18,"underline":19,"image":20,"link":21,"unlink":22,"close":23,"arrow":25,"upload":26}
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
	...
	init : function() {
		...
		this.elmFrame = new bkElement('iframe').setAttributes({'src' : 'javascript:;', 'frameBorder' : 1, 'allowTransparency' : 'true', 'scrolling' : 'no'}).setStyle({height: '100px', width: '100%'}).addClass('frame').appendTo(this.elm);
...
var nicUploadButton = nicEditorAdvancedButton.extend({
	...
	makeRequest : function() {
		if(this.pane && this.pane.pane) {
			nicUploadButton.lastPlugin = this;
			var s = new bkElement('script').setAttributes({ type : 'text/javascript', src : this.uri+'?check=11111111'+this.myID+'&rand='+Math.round(Math.random()*Math.pow(10,15))}).addEvent('load', function() {
